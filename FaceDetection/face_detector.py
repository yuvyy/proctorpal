...
def get_face_detector(modelFile= r"FaceDetection\models\res10_300x300_ssd_iter_140000.caffemodel",
                      configFile= r"FaceDetection\models\deploy.prototxt",
                      quantized=False):
    """
    ...
    if quantized:
        if modelFile == None:
            modelFile = r"models/opencv_face_detector_uint8.pb"
        if configFile == None:
            configFile = r"models/opencv_face_detector.pbtxt"
        model = cv2.dnn.readNetFromTensorflow(modelFile, configFile)
        
    else:
        if modelFile == None:
            modelFile = r"FaceDetection/models/res10_300x300_ssd_iter_140000.caffemodel"
        if configFile == None:
            configFile = r"FaceDetection/models/deploy.prototxt"
