...
import cv2
import numpy as np
from app.test.face_detector import get_face_detector, find_faces
from app.test.face_landmarks import get_landmark_model, detect_marks
...
# Initialize data lists to collect information
time_points = []  # Time points for each frame
left_eye_positions = []  # Store left eye positions
right_eye_positions = []  # Store right eye positions

while True:
    ret, img = cap.read()
    current_time = time.time()  # Get the current time
    time_points.append(current_time)  # Append the current time

# Add left and right eye positions to their respective lists
    left_eye_positions.append(eyeball_pos_left)
    right_eye_positions.append(eyeball_pos_right)


# After the loop, you can plot the data 
    plt.figure(figsize=(10, 6))
    plt.plot(time_points, left_eye_positions, label="Left Eye Position", color='red')
    plt.plot(time_points, right_eye_positions, label="Right Eye Position", color='blue')
